...
namespace XPX
{
	...
	ImageComponent::ImageComponent(const char* path, const char* parent, const char* name)
		: Lua::CLuaClass((String(parent) + "." + name)), mImage(nullptr)
	{
		this->mImage = CAD->getVideoDriver()->getTexture(path);

		this->insert("ClassName", name);
		this->insert("ImagePath", path);
		this->insert("Parent", parent);
	...
	const char* ImageComponent::name()
	{
		auto rect_name = this->index_as_string("ClassName");
		return ((!this->index_as_string("ClassName").empty()) ? rect_name.c_str() : "RectComponent");
	}
	...
	void ImageComponent::update(ClassPtr klass) noexcept
	{
		ImageComponent* the_image = (ImageComponent*)klass;

		if (the_image &&
			the_image->mImage)
		{
			int x = the_image->index_as_number("Left");
			int y = the_image->index_as_number("Top");
			int x2 = the_image->index_as_number("Right");
			int y2 = the_image->index_as_number("Bottom");

			auto clr = SColor(255, 255, 255, 255);

			CAD->getVideoDriver()->draw2DImage(the_image->mImage, recti(0, 0, 0, 0), recti(x, y, x2, y2), nullptr, &clr, true);
		}
	}


	bool ImageComponent::should_update() { return true; }
}