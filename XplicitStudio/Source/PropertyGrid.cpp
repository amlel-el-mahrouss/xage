...
LPCWSTR g_szClassName = _T("PropGridCtl"); ///< The classname.
...
{
    //DWM 1.3: Added
    return CallWindowProc((WNDPROC)GetPropW(hwnd, WPRC), hwnd, msg, wParam, lParam);
}
...
typedef struct tagLISTBOXITEM {
    LPWSTR lpszCatalog; ///< Catalog (group) name
    LPWSTR lpszPropName;///< Property (item) name
    LPWSTR lpszMisc;    ///< Item specific data string
    LPWSTR lpszPropDesc;///< Property (item) description
    LPWSTR lpszCurValue;///< Property (item) value
...
{
    *ppInstanceData = (LPINSTANCEDATA)GetPropW(hControl, (LPCWSTR)_T("lpInsData"));
...
static BOOL Control_CreateInstanceData(HWND hControl, LPINSTANCEDATA pInstanceData)
{
    ...
    return SetPropW(hControl, (LPCWSTR)_T("lpInsData"), pInst);
}
...
static BOOL Control_FreeInstanceData(HWND hControl)
{
    ...
    if (Control_GetInstanceData(hControl, &pInst))
    {
        ...
        RemovePropW(hControl, (LPCWSTR)_T("lpInsData"));
        return TRUE;
    }
    return FALSE;
}
...
static LPWSTR NewString(LPWSTR str)
{
    if (NULL == str || _T('\0') == *str) str = (wchar_t*)_T("");
    LPWSTR tmp = (LPWSTR)calloc(ua_wcslen(str) + 1, sizeof(WCHAR));

    if (NULL == tmp)
    {
        return (LPWSTR)calloc(1, sizeof(WCHAR));
    }
    return (LPWSTR)_tmemmove(tmp, str, ua_wcslen(str));
}
...
static LPWSTR NewStringArray(LPWSTR szzStr)
{
    if (NULL == szzStr || _T('\0') == *szzStr) szzStr = (wchar_t*)_T("");

    //Determine total buffer length
    INT iLen = 0;
    //Walk the buffer to find the terminating empty string.
    for (LPWSTR p = szzStr; *p; (p += wcslen(p) + 1, iLen = p - szzStr));

    //Allocate for array
    LPWSTR tmp = (LPWSTR)calloc(iLen + 1, sizeof(WCHAR));

    if (NULL == tmp)
    {
        return (LPWSTR)calloc(1, sizeof(WCHAR));
    }
    return (LPWSTR)_tmemmove(tmp, szzStr, iLen);
}
...
static LPLISTBOXITEM NewItem(LPWSTR szCatalog, LPWSTR szPropName, LPWSTR szCurValue, LPWSTR szMisc, LPWSTR szPropDesc, INT iItemType)
{
...
static VOID ListBox_OnKillFocus(HWND hwnd, HWND hwndNewFocus)
{
    if (NULL != g_lpInst->lpCurrent)
    {
        if (PIT_CHECK == g_lpInst->lpCurrent->iItemType)//DWM 1.3: Added
            g_lpInst->lpCurrent->lpszMisc = (wchar_t*)UNSELECT;
    }
...
static COLORREF GetColor(LPWSTR src)
{
    INT RVal, GVal, BVal;
    RVal = GVal = BVal = 0;
    if (wcslen(src) > 0)
        swscanf(src, _T("%d,%d,%d"), &RVal, &GVal, &BVal);
...
static HWND CreateListBox(HINSTANCE hInstance, HWND hwndParent, INT id)
{
    ...
    SetPropW(hwnd, WPRC, (HANDLE)GetWindowLongPtr(hwnd, GWLP_WNDPROC));
...
static LRESULT CALLBACK Edit_Proc(HWND hEdit, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        SetWindowLongPtr(hEdit, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hEdit, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hEdit, WPRC);
        return 0;
    }
    ...
    else if (WM_CHAR == msg && VK_RETURN == wParam)
    {
        ...
        {
            WCHAR buf[MAX_PATH];
            GetWindowTextW(hEdit, buf, sizeof buf);
...
static LRESULT CALLBACK IpEdit_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    GetClassNameW(hwnd, buf, NELEMS(buf));
    BOOL fEdit = (0 == _wcsicmp(buf, WC_EDITW));
    ...
    {
        SetWindowLongPtr(hwnd, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hwnd, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hwnd, WPRC);
        return 0;
    }
    ...
    else    //Handle messages (events) in the parent ipedit control
    {
        ...
        else if (WM_COMMAND == msg)
        {
            ...
            {
                WNDPROC lpfn = (WNDPROC)GetPropW(hwndCtl, WPRC);
                if (NULL == lpfn)
                {
                    //Subclass child and save the OldProc
                    SetPropW(hwndCtl, WPRC, (HANDLE)GetWindowLongPtr(hwndCtl, GWLP_WNDPROC));
...
static LRESULT CALLBACK Button_Proc(HWND hButton, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        SetWindowLongPtr(hButton, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hButton, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hButton, WPRC);
        return 0;
    }
    else if (WM_KILLFOCUS == msg)
    {
...
static LRESULT CALLBACK DatePicker_Proc(HWND hDate, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        SetWindowLongPtr(hDate, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hDate, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hDate, WPRC);
        return 0;
    }
    ...
    else if (WM_CHAR == msg && VK_RETURN == wParam)
    {
        if (NULL != g_lpInst->lpCurrent)
        {
            ...
            case PIT_DATE:
            {
                ...
                GetDateFormatW(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &st, NULL, buf, MAX_PATH);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
            ...
            case PIT_TIME:
            {
                ...
                WCHAR buf[MAX_PATH] = { 0 };
                DateTime_GetSystemtime(g_lpInst->hwndCtl1, &st);
                GetTimeFormatW(LOCALE_USER_DEFAULT, 0, &st, _T("hh':'mm':'ss tt"), buf, MAX_PATH);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
                ShowWindow(g_lpInst->hwndCtl1, SW_HIDE);
            }
            break;
            case PIT_DATETIME:
            {
                ...
                GetTimeFormatW(LOCALE_USER_DEFAULT, 0, &st, _T("hh':'mm':'ss tt"), (LPWSTR)(buf + wcslen(buf)), MAX_PATH - wcslen(buf));
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
                ShowWindow(g_lpInst->hwndCtl1, SW_HIDE);
                ShowWindow(g_lpInst->hwndCtl2, SW_HIDE);
            }
...
static LRESULT CALLBACK ComboBox_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    //  or the edit field's greatgrandparent
    GetClassNameW(hwnd, classname, NELEMS(classname));
    BOOL fEdit = (0 == _wcsicmp(classname, WC_EDITW));
    ...
    {
        SetWindowLongPtrW(hwnd, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hwnd, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hwnd, WPRC);
        return 0;
    }
    else if (WM_KILLFOCUS == msg)//DWM 1.3: Added
    ...
    else if (WM_COMMAND == msg)
    {
        ...
        HWND hwndCtl = GET_WM_COMMAND_HWND(wParam, lParam);
        {
            ...
            if (NULL == lpfn)
            {
                ...
                GetClassNameW(hwndCtl, classname, NELEMS(classname));
                if (0 == _wcsicmp(classname, WC_EDITW))
                {
    ...
    else if (WM_CHAR == msg && VK_RETURN == wParam)
    {
        if (NULL != g_lpInst->lpCurrent)
        {
            GetWindowTextW(hwnd, classname, sizeof classname);   //Combo or child edit text is the same
...
///
/// @returns VOID.
static VOID LogFontItem_FromString(LPPROPGRIDFONTITEM lpLogFontItem, LPWSTR lpszFont)
{
...
///
/// @returns LPWSTR The string representation of PROPGRIDFONTITEM elements.
static LPWSTR LogFontItem_ToString(LPPROPGRIDFONTITEM lpLogFontItem)
{
    static WCHAR buf[MAX_PATH];
    _tmemset(buf, (WCHAR)0, MAX_PATH);
...
///
/// @returns VOID.
static VOID FileDialogItem_FromString(LPPROPGRIDFDITEM lpPgFdItem, LPWSTR lpszFdItem)
{
    //DWM 1.2: Added method
    static WCHAR PgFdItem[4][MAX_PATH];
    memset(PgFdItem, (WCHAR)0, sizeof(PgFdItem));
...
#endif
        (LPWSTR)&PgFdItem[0], (LPWSTR)&PgFdItem[1], (LPWSTR)&PgFdItem[2], (LPWSTR)&PgFdItem[3]);

    for (int i = 0; i < NELEMS(PgFdItem); i++)
    {
        if (0 == _wcsicmp(_T("?"), PgFdItem[i]))
        {
            //Convert back to empty string
            PgFdItem[i][0] = (WCHAR)0;
        }
        else if (2 == i)
        {
            //Convert back to double null-terminated string
            for (LPWSTR ptr = PgFdItem[2]; *ptr; ptr++)
                if (_T('\t') == *ptr) *ptr = _T('\0');
        }
    }
...
///
/// @returns LPWSTR The string representation of PROPGRIDFDITEM elements.
static LPWSTR FileDialogItem_ToString(LPPROPGRIDFDITEM lpPgFdItem)
{
    static WCHAR szBuf[3 * MAX_PATH];
    _tmemset(szBuf, (WCHAR)0, NELEMS(szBuf));

    WCHAR filter[MAX_PATH] = { 0 };

    //Copy filter string replacing \0 with \t"
    INT iLen = 0;
    for (LPWSTR psz = (LPWSTR)lpPgFdItem->lpszFilter,
        ps = filter, pe = filter + NELEMS(filter) - 1;
        *psz && ps < pe; psz += iLen + 1)
    {
        _tmemmove(ps, psz, (iLen = wcslen(psz)));
        ps += iLen;
        *ps++ = _T('\t');
    }
    ...
    _stprintf(szBuf, NELEMS(szBuf),
        ...
        "Default Extension: %3s",
#endif
        0 < wcslen(lpPgFdItem->lpszDlgTitle) ? lpPgFdItem->lpszDlgTitle : _T("?"), //DWM 1.2: Added default "?"
        0 < wcslen(lpPgFdItem->lpszFilePath) ? lpPgFdItem->lpszFilePath : _T("?"), //DWM 1.2: Added default "?"
        0 < wcslen(filter) ? filter : _T("?"), //DWM 1.2: Added default "?"
        0 < wcslen(lpPgFdItem->lpszDefExt) ? lpPgFdItem->lpszDefExt : _T("?")); //DWM 1.2: Added default "?"
    return szBuf;
}
...
static LPITEMIDLIST ConvertPathToLpItemIdList(LPWSTR pszPath)
{
    LPITEMIDLIST pidl;
...
static BOOL CALLBACK BrowseCallbackProc(HWND hwnd, UINT uMsg, LPARAM lParam, LPARAM lpData)
{
    ...
    static WCHAR SelectedDir[MAX_PATH];
    ...
    case BFFM_SELCHANGED:
    {
        ...
        if (SHGetPathFromIDListW((LPITEMIDLIST)lParam, SelectedDir))
...
static LPWSTR BrowseFolder(HWND hwnd, LPWSTR curPath, LPWSTR title, LPWSTR rootPath)
{
    BROWSEINFOW bi;
    ...
    if (SUCCEEDED(SHGetMalloc(&pMalloc)))
    {
        ...
        if (pidl)
            ...
            _tmemset(szDir, (WCHAR)0, MAX_PATH);
...
static INT ListBox_FindCatalog(HWND hwnd, INT indexStart, LPCWSTR szCatalog)
{
    INT ln = wcslen(szCatalog) + 1;
    ...
    for (INT i = indexStart < 0 ? 0 : indexStart; i < ListBox_GetCount(hwnd); i++)
    {
        ...
        if (NULL != pItem && 0 == _wcsicmp(pItem->lpszCatalog, szCatalog))
            return i;
    }
    return LB_ERR;
}
...
static VOID ToggleCatalog(LPLISTBOXITEM pItem)
{
    ...
    if (pItem->fCollapsed)
    {
        for (INT i = ListBox_GetCount(g_lpInst->hwndListBox) - 1; i >= 0; i--)
        {
            ...
            if (NULL != p && _wcsicmp(p->lpszCatalog, pItem->lpszCatalog) == 0)
            {
    ...
    else
    {
        ...
        if (LB_ERR != idx)
        {
            ...
            for (INT i = ListBox_GetCount(g_lpInst->hwndListMap) - 1; 0 <= i; i--)
            {
                ...
                {
                    if (0 == _wcsicmp(pp->lpszCatalog, pItem->lpszCatalog))
                    {
...
static VOID ListBox_OnEndScroll(HWND hwnd)
{
    ...
    case PIT_CHECK:
        g_lpInst->lpCurrent->lpszMisc = (wchar_t*)SELECT;
    ...
    if ((x >= g_lpInst->iHDivider - 5) && (x <= g_lpInst->iHDivider + 5))
    {
        ...
        if (NULL != g_lpInst->lpCurrent)
        {
            ...
            case PIT_STATIC:
                break; //Ignore
            case PIT_CHECK:
                g_lpInst->lpCurrent->lpszMisc = (wchar_t*)UNSELECT; //Prevent toggle
                break;
            case PIT_DATETIME:
                //DWM 1.6: force update of grid data
                GetWindowTextW(g_lpInst->hwndCtl2, buf, sizeof buf);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
                ShowWindow(g_lpInst->hwndCtl2, SW_HIDE);
                // Fall through
            default:
                //DWM 1.6: force update of grid data
                GetWindowTextW(g_lpInst->hwndCtl1, buf, sizeof buf);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
                ShowWindow(g_lpInst->hwndCtl1, SW_HIDE);
                break;
            }
...
static VOID ListBox_OnLButtonUp(HWND hwnd, INT x, INT y, UINT keyFlags)
{
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        ...
        case PIT_STATIC:
            return;
        case PIT_CHECK:
            if (0 == _wcsicmp(g_lpInst->lpCurrent->lpszMisc, SELECT))
            {
    ...
    if (NULL != g_lpInst->hwndToolTip)
    {
        ...
        if ((x >= g_lpInst->iHDivider + 5) && !g_lpInst->fTracking)
        {
            ...
            if (NULL != pItem)
            {
                ...
                default:
                    //Replace the text in the buf and update the tool
                    wcsncpy(newText, pItem->lpszCurValue, NELEMS(newText) - 1);
                    break;
                }
            }
        }
        if (0 != wcsncmp(buf, newText, wcslen(newText))) //DWM 1.4: Do not update unless text changed to reduce flicker
        {
            //Clear out old text
            _tmemset(buf, (WCHAR)0, NELEMS(buf) - 1);
            //Replace the text in the buf and update the tool
            wcsncpy(buf, newText, NELEMS(buf) - 1);
...
static VOID ListBox_OnSelectEdit(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    SetWindowTextW(g_lpInst->hwndCtl1, pItem->lpszCurValue);
    ShowWindow(g_lpInst->hwndCtl1, SW_SHOW);
    SetFocus(g_lpInst->hwndCtl1);
...
static VOID ListBox_OnSelectIP(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    //DWM 1.9: This was necessary to support MSVC swscanf (sscanf - swscanf) beahavior which expects DWORD arguments.
    DWORD ip[4] = { 0, 0, 0, 0 };
    swscanf(pItem->lpszCurValue, _T("%hhu.%hhu.%hhu.%hhu"), &ip[0], &ip[1], &ip[2], &ip[3]);
...
static VOID ListBox_OnSelectDateTime(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    if (PIT_DATE == pItem->iItemType)
    {
        ...
        if (NULL != pItem)
            swscanf(pItem->lpszCurValue, _T("%hd/%hd/%hd"), &st.wMonth, &st.wDay, &st.wYear);
    ...
    else if (PIT_TIME == pItem->iItemType)
    {
...
#ifdef _UNICODE
        szFormat = (wchar_t*)_T("%hd:%hd:%hd %2ls");
#else
        ...
        if (NULL != pItem)
        {
            ...
            GetLocalTime(&st);
            swscanf(pItem->lpszCurValue, szFormat, &st.wHour, &st.wMinute, &st.wSecond, &buf);
        }
        if ((0 == _wcsicmp(_T("PM"), buf)) && st.wHour != 12)//DWM 1.6:Added st.wHour != 12
            st.wHour += 12;
...
#ifdef _UNICODE
        szFormat = (wchar_t*)_T("%hd/%hd/%hd %hd:%hd:%hd %2ls");
#else
        szFormat = _T("%hd/%hd/%hd %hd:%hd:%hd %2s");
#endif

        if (NULL != pItem)
            swscanf(pItem->lpszCurValue, szFormat, &st.wMonth, &st.wDay, &st.wYear, &st.wHour, &st.wMinute, &st.wSecond, &buf);
...
static VOID ListBox_OnSelectComboBox(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    for (LPWSTR p = pItem->lpszMisc; *p; p += wcslen(p) + 1)
    {
    ...
    else
    {
        ...
        SetWindowTextW(hCombo, pItem->lpszCurValue);
...
static VOID ListBox_OnKeyDown(HWND hwnd, UINT vk, BOOL fDown, INT cRepeat, UINT flags)
{
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        ...
        if (PIT_CHECK == g_lpInst->lpCurrent->iItemType && (VK_SPACE == vk || VK_RETURN == vk))
        {
            if (0 == _wcsicmp(g_lpInst->lpCurrent->lpszMisc, SELECT))
            {
                g_lpInst->lpCurrent->lpszCurValue = (wchar_t*)(0 == _wcsicmp(g_lpInst->lpCurrent->lpszCurValue, CHECKED) ? UNCHECKED : CHECKED);
        ...
        else if (PIT_CHECK == g_lpInst->lpCurrent->iItemType && VK_ESCAPE == vk)
        {
            ...
            0 == _wcsicmp(g_lpInst->lpCurrent->lpszMisc, SELECT))
        {
            fHandled = TRUE;
        }
        else
        {
            if (VK_TAB == vk)
            {
                ...
                case PIT_CHECK:
                    ...
                    else if (0 == _wcsicmp(g_lpInst->lpCurrent->lpszMisc, SELECT))
                    {
                        g_lpInst->lpCurrent->lpszMisc = (wchar_t*)UNSELECT;
                        SetFocusToParent();
                    }
...
static VOID ListBox_OnCommand(HWND hwnd, INT id, HWND hwndCtl, UINT codeNotify)
{
    if (ID_BUTTON == id)
    {
        ...
        case PIT_FILE:
        {
            ...
            swscanf(g_lpInst->lpCurrent->lpszMisc,
#ifdef _UNICODE
                L"Title: %32l[^\r\n] " \
                L"Path: %256l[^\r\n] " \
                L"Filter: %256l[^\r\n] " \
                ...
                "Default Extension: %3[^\r\n]",
#endif
                (LPWSTR)&title, (LPWSTR)&path, (LPWSTR)&filter, (LPWSTR)&ext);

            if (0 != wcscmp(_T("?"), title)) //DWM 1.2: Added test
                ofn.lpstrTitle = title;
            else
                ofn.lpstrTitle = _T("Select file");

            if (0 != wcscmp(_T("?"), path)) //DWM 1.2: Added test
            {
                //Exclude the filename
                for (LPWSTR ptr = path + wcslen(path) - 1; *ptr; ptr--)
                    if (*ptr == _T('\\'))
                    {
                        *ptr = _T('\0');
                        break;
            ...
            if (0 != wcscmp(_T("?"), filter)) //DWM 1.2: Added test
            {
                //Convert back to double null-terminated string
                for (LPWSTR ptr = filter; *ptr; ptr++)
                    if (_T('\t') == *ptr)
                        *ptr = _T('\0');
            ...
            if (0 != wcscmp(_T("?"), ext)) //DWM 1.2: Added test
            {
                ofn.lpstrDefExt = ext;
            }
            else
            ...
            {
                PROPGRIDFDITEM pgi;
                pgi.lpszDlgTitle = (LPWSTR)ofn.lpstrTitle;
                pgi.lpszFilePath = (LPWSTR)ofn.lpstrFile;
                pgi.lpszFilter = (LPWSTR)ofn.lpstrFilter;
                pgi.lpszDefExt = (LPWSTR)ofn.lpstrDefExt;
        ...
        case PIT_FOLDER:
        {
            LPWSTR temp = BrowseFolder(hwnd, g_lpInst->lpCurrent->lpszCurValue, (wchar_t*)_T(""), (wchar_t*)_T(""));
            //DWM 1.2: Reset to unselected folder //if (0 < wcslen(temp))
...
///
/// @returns VOID.
static VOID Grid_ExpandCatalog(LPCWSTR szCatalog)
{
    if (NULL != szCatalog)
    {
        INT ln = wcslen(szCatalog) + 1; //Check for empty string
        if (ln == 1)
            return;
    }
    ...
    for (INT i = 0; i < ListBox_GetCount(g_lpInst->hwndListBox); i++)
    {
        ...
        if (NULL != pItem && PIT_CATALOG == pItem->iItemType &&
            (NULL == szCatalog || 0 == _wcsicmp(pItem->lpszCatalog, szCatalog)))
        {
            if (pItem->fCollapsed)
                ToggleCatalog(pItem);
        }
...
///
/// @returns VOID.
static VOID Grid_CollapseCatalog(LPCWSTR szCatalog)
{
    if (NULL != szCatalog)
    {
        INT ln = wcslen(szCatalog) + 1; //Check for empty string
...
static VOID Grid_OnCommand(HWND hwnd, INT id, HWND hwndCtl, UINT codeNotify)
{
    if (g_lpInst->hwndListBox == hwndCtl)
    {
        ...
        case LBN_SELCHANGE:
        {
            ...
            if (NULL != pItem)
            {
                if (PIT_CHECK == pItem->iItemType)
                {
                    if (iCurSel != g_lpInst->iPrevSel &&
                        0 == _wcsicmp(pItem->lpszMisc, SELECT))
                        pItem->lpszMisc = (wchar_t*)UNSELECT;
                }
            }
            ...
            if (NULL != g_lpInst->hwndPropDesc)
                SetWindowTextW(g_lpInst->hwndPropDesc,
...
static VOID Grid_OnDrawItem(HWND hwnd, const DRAWITEMSTRUCT* lpDIS)
{
    ...
    if (PIT_CATALOG == pItem->iItemType)
    {
        ...
        DrawTextW(lpDIS->hDC, pItem->lpszCatalog, wcslen(pItem->lpszCatalog),
    ...
    else
    {
        ...
        DrawTextW(lpDIS->hDC, pItem->lpszPropName, wcslen(pItem->lpszPropName),
    ...
    if (PIT_CHECK != pItem->iItemType)
    {
        ...
        if (PIT_FONT == pItem->iItemType)
        {
            ...
            if (0 < PointSize)
            {
                ...
                DrawTextW(lpDIS->hDC, buf, wcslen(buf),
        ...
        else
        {
            ...
            DrawTextW(lpDIS->hDC, pItem->lpszCurValue, wcslen(pItem->lpszCurValue),
                MAKE_PRECT(rectPart3.left + 3, rectPart3.top + 3,
                    rectPart3.right + 3, rectPart3.bottom + 3),
                DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
        }
...
static INT Grid_OnAddString(LPPROPGRIDITEM pgi)
{
    ...
    LPWSTR lpszCurValue = (wchar_t*)_T("");
    ...
    {
        lpi = NewItem(pgi->lpszCatalog, (wchar_t*)_T(""), (wchar_t*)_T(""),
            (wchar_t*)_T(""), (wchar_t*)_T(""), PIT_CATALOG);
        lpi->fCollapsed = TRUE;
    ...
    case PIT_EDITCOMBO:
    case PIT_FOLDER:
        lpszCurValue = (LPWSTR)pgi->lpCurValue;
        break;
    case PIT_COLOR:
    ...
    case PIT_FONT:
        ...
        break;
    case PIT_CHECK:
        lpszCurValue = (BOOL)pgi->lpCurValue ? (wchar_t*)CHECKED : (wchar_t*)UNCHECKED;
        break;
    case PIT_FILE:
    ...
    case PIT_DATE:
        ...
        break;
    case PIT_TIME:
        GetTimeFormatW(LOCALE_USER_DEFAULT, 0, (LPSYSTEMTIME)pgi->lpCurValue,
    ...
    case PIT_DATETIME:
        ...
        wcscat(buf, _T(" "));
        GetTimeFormatW(LOCALE_USER_DEFAULT, 0, (LPSYSTEMTIME)pgi->lpCurValue,
            _T("hh':'mm':'ss tt"), (LPWSTR)(buf + wcslen(buf)),
            MAX_PATH - wcslen(buf));
        lpszCurValue = buf;
        break;
    }
    ...
    if (NULL != pItem)
    {
        if (PIT_CATALOG != pItem->iItemType)
        {
            ...
            case PIT_FONT:
                ...
                break;
            case PIT_CHECK:
                pgi.lpCurValue = (LPARAM)(0 == _wcsicmp(CHECKED,
                    pItem->lpszCurValue));
                break;
            case PIT_FILE:
            ...
            case PIT_IP:
            {
                ...
                //DWM 1.9: This was necessary to support MSVC swscanf (sscanf - swscanf) beahavior which expects DWORD arguments.
                DWORD ip[4] = { 0, 0, 0, 0 };
                swscanf(pItem->lpszCurValue, _T("%hhd.%hhd.%hhd.%hhd"),
                    &ip[0], &ip[1], &ip[2], &ip[3]);
            ...
            case PIT_DATE:
                memset(&st, 0, sizeof(SYSTEMTIME));
                swscanf(pItem->lpszCurValue, _T("%hd/%hd/%hd"),
                    &st.wMonth, &st.wDay, &st.wYear);
                pgi.lpCurValue = (LPARAM)&st;
                break;
            case PIT_TIME:
                memset(&st, 0, sizeof(SYSTEMTIME));
                _tmemset(outbuf, (WCHAR)0, MAX_PATH);
                swscanf(pItem->lpszCurValue,
#ifdef _UNICODE
                    _T("%hd:%hd:%hd %2ls"),
#else
                    _T("%hd:%hd:%hd %2s"),
#endif
                    & st.wHour, &st.wMinute, &st.wSecond, (LPWSTR)&outbuf);
                if ((0 == _wcsicmp(_T("PM"), outbuf)) && st.wHour != 12)//DWM 1.6:Added st.wHour != 12
                    st.wHour += 12;
...
static VOID Grid_OnResetContent(VOID)
{
    ...
    SetWindowTextW(g_lpInst->hwndPropDesc, _T("")); //DWM 1.2: Clear the property pane
}
    ...
    if (NULL != pItem)
    {
        if (PIT_CATALOG != pItem->iItemType)
        {
            ...
            if (LB_ERR != i)
            {
                ...
                if (PIT_CHECK == pItem->iItemType)
                    pItem->lpszMisc = (wchar_t*)SELECT; //DWM 1.2: Ensure Checkbox selected
...
ATOM InitPropertyGrid(HINSTANCE hInstance)
{
    ...
    if (!GetClassInfoExW(NULL, L"ListBox", &wcex))
        return 0;

    // Add our own stuff
    wcex.lpfnWndProc = (WNDPROC)Grid_Proc;;
    wcex.hInstance = hInstance;
    wcex.lpszClassName = (wchar_t*)g_szClassName;
...
HWND New_PropertyGrid(HWND hParent, DWORD dwID)
{
    ...
    hPropertyGrid = CreateWindowExW(0, g_szClassName, _T(""),
    ...
    WPARAM wParam, LPARAM lParam)
{
    ...
    {
        SetWindowLongPtr(hList, GWLP_WNDPROC, (DWORD_PTR)GetPropW(hList, WPRC)); //DWM 1.5: fixed cast
        RemovePropW(hList, WPRC);
        return 0;
    }
    }
...
static LRESULT CALLBACK Grid_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        if (NULL == (LPWSTR)lParam) //Expand all
        {
        ...
        else
        {
            ...
            for (LPWSTR p = (LPWSTR)lParam; *p; p += wcslen(p) + 1)
                Grid_ExpandCatalog((LPCWSTR)p);
        }
    }
    break;
    case PG_COLLAPSECATALOGS:
    {
        if (NULL == (LPWSTR)lParam) //Collapse all
        {
        ...
        {
            for (LPWSTR p = (LPWSTR)lParam; *p; p += wcslen(p) + 1)
                Grid_CollapseCatalog((LPCWSTR)p);
        }
    }
    break;
